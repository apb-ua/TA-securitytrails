...
import modalert_list_tags_helper

class AlertActionWorkerlist_tags(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerlist_tags, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_list_tags_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerlist_tags("TA-securitytrails", "list_tags").run(sys.argv)
    sys.exit(exitcode)
