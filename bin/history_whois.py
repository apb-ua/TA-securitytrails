...
import modalert_history_whois_helper

class AlertActionWorkerhistory_whois(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerhistory_whois, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_history_whois_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerhistory_whois("TA-securitytrails", "history_whois").run(sys.argv)
    sys.exit(exitcode)
