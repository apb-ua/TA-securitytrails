...
import modalert_get_associated_helper

class AlertActionWorkerget_associated(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerget_associated, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_get_associated_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerget_associated("TA-securitytrails", "get_associated").run(sys.argv)
    sys.exit(exitcode)
