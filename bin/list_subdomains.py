...
import modalert_list_subdomains_helper

class AlertActionWorkerlist_subdomains(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerlist_subdomains, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_list_subdomains_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerlist_subdomains("TA-securitytrails", "list_subdomains").run(sys.argv)
    sys.exit(exitcode)
